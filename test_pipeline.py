...
def parse_args():
    ...
    p.add_argument("--run-key",   default="abstract",
                   choices=["abstract", "title", "combined"])
    p.add_argument("--show-api",  action="store_true",
                   help="Print available API endpoints and exit")
    p.add_argument("--logs",      action="store_true",
                   help="Stream Space container logs to stdout")
    p.add_argument("--status",    action="store_true",
                   help="Print Space build/runtime status")
    return p.parse_args()
...
FILLERS = {
    "aspect":    ["perceived usefulness", "interface design", "social influence", "personalisation"],
    "tech":      ["AI recommendations", "blockchain", "augmented reality", "chatbots", "IoT"],
    "factor":    ["trust", "ease of use", "perceived risk", "social norms", "habit"],
    "outcome":   ["satisfaction", "loyalty", "engagement", "conversion rate", "retention"],
    "n":         ["350", "420", "512", "278", "634", "189"],
    "behavior":  ["impulsive buying", "review generation", "subscription renewal", "cart abandonment"],
    "method":    ["convolutional neural network", "transformer model", "hybrid approach", "ensemble"],
    "task":      ["sentiment analysis", "fraud detection", "demand forecasting", "anomaly detection"],
    "domain":    ["customer reviews", "financial transactions", "social media data", "log files"],
    "framework": ["multi-layer architecture", "federated learning approach", "attention mechanism"],
    "benchmark": ["standard IS datasets", "cross-industry evaluations", "held-out test sets"],
    "approach":  ["the proposed system", "our integrated model", "the hybrid algorithm"],
    "metric":    ["accuracy", "F1 score", "AUC-ROC", "precision-recall"],
    "model":     ["predictive model", "classification framework", "decision support system"],
    "tech1":     ["natural language processing", "graph neural networks", "transfer learning"],
    ...
    "protocol":  ["end-to-end encryption protocol", "zero-trust framework", "RBAC scheme"],
    "attack":    ["man-in-the-middle", "credential stuffing", "social engineering"],
    "practice":  ["security awareness training", "multi-factor authentication", "patching"],
    ...
    "context":   ["emerging markets", "enterprise settings", "SMEs", "educational institutions"],
...
def check_status(space_id: str, token: str):
    ...
    try:
        ...
        if hasattr(runtime, 'sdk'):     print(f"  SDK:      {runtime.sdk}")
        if hasattr(runtime, 'hardware'):print(f"  Hardware: {runtime.hardware}")
    except Exception as e:
        warn(f"Could not read runtime: {e}")
    try:
        info_obj = api.space_info(space_id)
        if info_obj.last_modified: print(f"  Modified: {info_obj.last_modified}")
        if info_obj.sha:           print(f"  SHA:      {info_obj.sha[:12]}")
    except Exception as e:
    ...
    try:
        with requests.get(url, headers=headers, stream=True, timeout=30) as r:
            if r.status_code == 403:
                fail("403 — token can't read logs (needs at least read access to the Space repo)")
                return
            ...
            for raw_line in r.iter_lines():
                ...
                if decoded.startswith("data: "):
                    try:
                        ...
                        colour = RED if level == "ERROR" else (YELLOW if level == "WARNING" else "")
...
def smoke_test(space_id: str, csv_path: str, hf_token: str, run_key: str):
    ...
    info(f"  Expected time: 3–6 min. Labelling now batches at 15 topics to stay under Groq TPM.")
    t = time.time()
    try:
        result = client.predict(
            f"run {run_key}",
    ...
    try:
        ...
        if total == 0:
            ...
            warn(f"{unlabeled}/{total} topics are unlabeled (>15% fallback rate, check Groq rate limits)")
        else:
...
def main():
    ...
    if args.status:
        if not token:
            print("Need --log-token or --hf-token"); sys.exit(1)
    ...
    if args.show_api:
        if not args.hf_token:
            print("Need --hf-token to connect"); sys.exit(1)
        show_api(args.space_id, args.hf_token)
        return

    # ── Smoke test mode ────────────────────────────────────────────────────
    if not args.hf_token:
        print("Need --hf-token to run the smoke test"); sys.exit(1)

    if args.synth:
        tmp = tempfile.NamedTemporaryFile(
            suffix="_ElectronicMarkets_TopicModelling_Export.csv",
            delete=False
        )
